...
def check_dev_env_name_not_exist(dev_env_local_setup: DevEnvLocalSetup, new_dev_env_name: str) -> (int | None):
    if dev_env_local_setup.get_dev_env_by_name(new_dev_env_name):
        stderr.print("[red]Error: The input Development Environment exist.[/]")
        return -1
    else:
        return True    

def clone_given_dev_env(derserialized_local_dev_nev: dict, dev_env_name: str, new_dev_end_name: str) -> (int | None):
    new_devenv=""
    ...
    new_devenv["name"] = new_dev_end_name
    print(new_devenv) 
    print("**************")  
    derserialized_local_dev_nev["development_environments"].append(new_devenv)
    print(derserialized_local_dev_nev["development_environments"])
...
def execute(dev_env_name: str, new_dev_env_name: str) -> None:
    ...
    if (check_dev_env_name_exist(dev_env_local_setup,dev_env_name) != -1 and 
        check_dev_env_name_not_exist(dev_env_local_setup,new_dev_env_name) == True):
